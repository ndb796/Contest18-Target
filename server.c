/* https://raw.githubusercontent.com/eugeneli/Multithreaded-Chat-in-C/master/server.c */
...
void chatserver(){
    ...
    err = pthread_create(&tid1, NULL, startchatserver, NULL);
    if (err != 0) exit(err);
...
/* int chatserver(int argc, char *argv[]) */
int startchatserver() 
{
...
void *messageHandler(void *data)
{
	...
	int i;
        for(i = 0; i < chatData->numClients; i++)
        {
